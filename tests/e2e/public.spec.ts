...
test("homepage không có cart hoặc checkout", async ({ page }) => { await page.goto("/"); await expect(page.getByRole("heading", { level: 1 })).toContainText("Tìm đúng sản phẩm"); await expect(page.getByText(/thêm vào giỏ/i)).toHaveCount(0); await expect(page.getByText(/checkout|thanh toán online/i)).toHaveCount(0); });
