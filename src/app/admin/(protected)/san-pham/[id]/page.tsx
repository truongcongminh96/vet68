...
export default async function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div><h1 className="text-3xl font-extrabold">Chỉnh sửa sản phẩm</h1><p className="mt-2 text-muted-foreground">{product.name} | {product.sku}</p><div className="mt-6"><ProductEditorForm defaultValues={defaults} brands={taxonomy.brands} categories={taxonomy.categories.filter((item) => item.kind === "product_type")} treatmentCategories={taxonomy.categories.filter((item) => item.kind === "treatment_need")} animalTypes={taxonomy.animalTypes} action={saveProductAction} initialImages={record.images} /></div></div>;
}
