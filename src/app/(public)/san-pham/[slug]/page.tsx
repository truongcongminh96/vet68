...
export default async function ProductDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="paper-page pb-28 md:pb-20">
      ...
      <div className="grid gap-9 lg:grid-cols-[0.94fr_1.06fr] lg:gap-14">
        <div><div className="paper-product-image relative aspect-square overflow-hidden rounded-[25%_10%_22%_8%_/_13%_22%_11%_20%] border-[7px] border-white shadow-[6px_8px_0_rgba(6,45,62,0.1)]"><Image src={product.images[0].src} alt={product.images[0].alt} fill priority sizes="(max-width: 1024px) 100vw, 48vw" className="object-contain p-5 md:p-8" /></div>{product.images.length > 1 ? <div className="mt-3 grid grid-cols-4 gap-3">{product.images.slice(1).map((image) => <div key={image.src} className="paper-panel relative aspect-square overflow-hidden"><Image src={image.src} alt={image.alt} fill sizes="120px" className="object-contain p-2" /></div>)}</div> : null}{demoMode ? <p className="mt-3 text-xs text-muted-foreground">Hình ảnh demo là minh hoạ bố cục, không phải bao bì sản phẩm đang kinh doanh.</p> : null}</div>
        <div>
          ...
          <p className="paper-eyebrow mt-6">Thông tin sản phẩm</p>
          <h1 className="paper-heading mt-3 text-3xl md:text-5xl">{product.name}</h1>
          ...
          <div className="paper-panel-warm mt-6 p-5"><ProductPrice product={product} /><p className="mt-3 text-xs leading-5 text-muted-foreground">Giá hiển thị chỉ mang tính tham khảo và có thể thay đổi. Hãy liên hệ Vet68 để xác nhận giá hiện tại.</p></div>
          ...
          <div className="paper-note mt-6 p-5"><p className="text-sm font-bold">Khi liên hệ, vui lòng ghi:</p><p className="mt-2 text-sm leading-6 text-muted-foreground">{product.name} | {product.sku} | {product.packaging}</p></div>
      ...
      <div className="mt-12 grid gap-8 lg:grid-cols-[1fr_0.34fr]">
        <Tabs defaultValue="information" className="paper-panel min-w-0 p-4 sm:p-6">
          <TabsList className="!grid !h-auto w-full grid-cols-1 gap-2 rounded-[18px_10px_18px_10px] bg-[#fff2df] p-1.5 sm:grid-cols-3">
            <TabsTrigger value="information" className="!h-14 !min-h-14 rounded-xl px-3 py-2 text-sm font-extrabold data-active:bg-[#fffaf0] sm:text-base"><ClipboardList aria-hidden="true" /> Thông tin sản phẩm</TabsTrigger>
            <TabsTrigger value="usage" className="!h-14 !min-h-14 rounded-xl px-3 py-2 text-sm font-extrabold data-active:bg-[#fffaf0] sm:text-base"><PackageCheck aria-hidden="true" /> Hướng dẫn sử dụng</TabsTrigger>
            <TabsTrigger value="storage" className="!h-14 !min-h-14 rounded-xl px-3 py-2 text-sm font-extrabold data-active:bg-[#fffaf0] sm:text-base"><Archive aria-hidden="true" /> Hướng dẫn bảo quản</TabsTrigger>
          </TabsList>
        ...
        <aside className="paper-panel-warm h-fit p-6"><AlertTriangle className="size-7 text-medical-red" aria-hidden="true" /><h2 className="mt-4 text-xl font-extrabold">Lưu ý an toàn</h2><p className="mt-3 text-sm leading-6 text-muted-foreground">{product.safetyInformation}</p><p className="mt-4 text-sm font-semibold text-medical-red">Thông tin website không thay thế tư vấn của bác sĩ thú y hoặc hướng dẫn từ nhà sản xuất.</p><Button variant="outline" className="mt-5 w-full bg-white" asChild><a href={contact.zaloUrl} target="_blank" rel="noreferrer"><MessageCircle aria-hidden="true" /> Tư vấn qua Zalo</a></Button></aside>
      </div>
...
function ProductSpecification({ label, value }: { label: string; value: string }) {
  return <div className="paper-panel p-4"><dt className="text-xs font-extrabold uppercase tracking-[0.08em] text-petshop-teal">{label}</dt><dd className="mt-2 line-clamp-3 text-sm font-semibold leading-6 text-foreground">{value}</dd></div>;
}
