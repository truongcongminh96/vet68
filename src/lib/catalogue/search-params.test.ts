...
describe("parseCatalogueFilters", () => {
  ...
  it("đọc bộ lọc công ty phân phối", () => { expect(parseCatalogueFilters({ company: "company-biovet-demo" })).toMatchObject({ company: "company-biovet-demo" }); });
  ...
  it("đọc thứ tự tên Z-A", () => { expect(parseCatalogueFilters({ sort: "name_desc" }).sort).toBe("name_desc"); });
});
