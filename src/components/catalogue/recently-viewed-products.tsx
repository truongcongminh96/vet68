...
export function RecentlyViewedProducts({ currentSlug, products }: { currentSlug: string; products: Product[] }) {
  ...
  return <section className="mt-12"><h2 className="text-2xl font-extrabold">Sản phẩm đã xem gần đây</h2><div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{items.map((product) => <Link key={product.id} href={`/san-pham/${product.slug}`} className="grid grid-cols-[72px_1fr] gap-3 rounded-xl border bg-card p-3"><span className="relative aspect-square overflow-hidden rounded-lg"><Image src={product.images[0].src} alt={product.images[0].alt} fill sizes="72px" className="object-cover" /></span><span><strong className="line-clamp-2 text-sm leading-5">{product.name}</strong><span className="mt-1 block text-xs text-muted-foreground">{product.sku}</span></span></Link>)}</div></section>;
}
