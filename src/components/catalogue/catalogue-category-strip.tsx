  ...
  return (
    <section className="mt-8" aria-labelledby="catalogue-categories-title">
      <div className="flex items-end justify-between gap-4">
        <div>
          <p className="text-xs font-extrabold uppercase tracking-[0.14em] text-[#257493]">Tìm nhanh theo nhóm</p>
          <h2 id="catalogue-categories-title" className="mt-1 text-2xl font-bold tracking-[-0.03em] text-primary md:text-3xl">Danh mục sản phẩm</h2>
        </div>
      ...
      <div className="mt-4 grid snap-x snap-mandatory auto-cols-[235px] grid-flow-col gap-3 overflow-x-auto overscroll-x-contain pb-3 sm:auto-cols-[260px] [scrollbar-color:#75c7e3_transparent] [scrollbar-width:thin]" aria-label="Các danh mục sản phẩm">
        {categories.map(({ category, productCount, image, imageAlt }) => (
          <Link key={category.id} href={`/danh-muc/${category.slug}`} className="group grid min-h-24 snap-start grid-cols-[76px_minmax(0,1fr)] items-center gap-3 rounded-xl border border-border bg-white p-3 transition-[border-color,box-shadow,transform] hover:-translate-y-0.5 hover:border-action hover:retail-card-shadow motion-reduce:transform-none">
